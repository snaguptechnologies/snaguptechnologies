...
async function initializeTables() {
  try {
    ...
    // Migration: rename any old C++ or Embedded Program courses to Embedded Programming
    try {
      await connection.query("UPDATE courses SET name = 'Embedded Programming' WHERE name LIKE '%C++%' OR name = 'Embedded Program'");
    } catch (mErr) {
    ...
    for (const cData of targetCourses) {
      try {
        const [cRows] = await connection.query("SELECT id FROM courses WHERE name = ?", [cData.name]);
        let courseId;
        if (cRows.length === 0) {
        ...
        const [bRows] = await connection.query(
          "SELECT id FROM batches WHERE course_id = ? AND batch_status IN ('upcoming', 'active')",
          [courseId]
        );
        if (bRows.length === 0) {
