...
const notifyAdminNewApplication = async (appData) => {
    let adminEmail = process.env.SNAGUP_ADMIN_EMAIL || process.env.ADMIN_EMAIL;
    if (!adminEmail) {
        try {
            const [rows] = await db.execute(`SELECT \`value\` FROM settings WHERE \`key\` = 'contact_email'`);
            if (rows.length > 0 && rows[0].value) adminEmail = rows[0].value;
        } catch(e) {}
    }
    if (!adminEmail) adminEmail = 'snaguptechnologies@gmail.com';

    const courseName = appData.course_name || 'Course';
    const subject = `New Course Application - ${courseName}`;
...
Application ID: ${appData.app_id || `APP-${appData.id}`}
Student Name: ${appData.student_name}
Phone Number: ${appData.phone}
Email: ${appData.email}
College Name: ${appData.college_name}
College Register ID: ${appData.college_register_id || 'N/A'}
WhatsApp Number: ${appData.whatsapp_number || appData.phone}
Selected Course: ${appData.course_name}
Application Status: ${appData.status || 'Applied'}
Application Date: ${appDate}
Application Time: ${appTime}
`.trim();

    const htmlTemplate = `
        <div style="font-family: 'Segoe UI', Helvetica, Arial, sans-serif; max-width: 650px; margin: 0 auto; padding: 40px; border: 2px solid #000000; background-color: #ffffff; color: #000000;">
            <div style="text-align: center; border-bottom: 2px solid #000000; padding-bottom: 20px; margin-bottom: 30px;">
                <h1 style="margin: 0; font-size: 24px; font-weight: 900; letter-spacing: -0.025em; text-transform: uppercase;">Snagup Technologies</h1>
                <p style="margin: 5px 0 0 0; font-size: 11px; font-weight: 700; letter-spacing: 0.2em; text-transform: uppercase;">Admissions Notification</p>
            </div>

            <div style="background-color: #000000; color: #ffffff; padding: 25px; margin-bottom: 30px;">
                <h2 style="font-size: 20px; font-weight: 900; margin: 0 0 8px 0; text-transform: uppercase;">New Course Application Received</h2>
                <p style="font-size: 14px; margin: 0; opacity: 0.9;">Course: <strong>${appData.course_name}</strong></p>
            </div>

            <div style="border: 2px solid #000000; padding: 25px; margin-bottom: 30px;">
                <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                    <tr><td style="padding: 8px 0; font-weight: 700; width: 160px; border-bottom: 1px solid #eee;">Application ID:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appData.app_id || `APP-${appData.id}`}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">Student Name:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appData.student_name}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">Phone Number:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appData.phone}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">Email:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appData.email}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">College Name:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appData.college_name}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">College Register ID:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appData.college_register_id || 'N/A'}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">WhatsApp Number:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appData.whatsapp_number || appData.phone}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">Selected Course:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;"><strong>${appData.course_name}</strong></td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">Application Status:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;"><span style="background: #ecfdf5; color: #047857; padding: 2px 8px; border-radius: 4px; font-weight: bold;">${appData.status || 'Applied'}</span></td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700; border-bottom: 1px solid #eee;">Application Date:</td><td style="padding: 8px 0; border-bottom: 1px solid #eee;">${appDate}</td></tr>
                    <tr><td style="padding: 8px 0; font-weight: 700;">Application Time:</td><td style="padding: 8px 0;">${appTime}</td></tr>
                </table>
            </div>

            <p style="font-size: 12px; color: #666; text-align: center; margin: 0;">This application has been saved to the MySQL database and logged in the central Excel report.</p>
        </div>
    `;
    ...
    await sendEmail({
        ...
        body: plainBody,
        html: htmlTemplate,
        purpose: 'New Course Application'
    });
};
