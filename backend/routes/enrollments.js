...
router.post('/', authenticateToken, requireRole('student'), async (req, res) => {
    ...
    try {
        ...
        if (!transaction_id || transaction_id.trim().length < 6) {
            throw new Error('Valid 12-digit UTR (Transaction ID) is required for UPI payments.');
        }
        ...
        const [existingTxs] = await connection.execute('SELECT id FROM payments WHERE transaction_id = ?', [transaction_id]);
        if (existingTxs.length > 0) throw new Error('This Transaction ID has already been used.');
        ...
        await connection.execute(`
            ...
            VALUES (?, ?, ?, 'upi', ?, 'pending')
        `, [enrollmentId, student_id, batch.price, transaction_id]);
