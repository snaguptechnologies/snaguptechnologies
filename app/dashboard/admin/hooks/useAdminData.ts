...
export const useAdminData = () => {
    ...
    const [releaseCertModal, setReleaseCertModal] = useState<{
        show: boolean;
        student_id: number;
        student_name: string;
        course_id?: number;
    ...
    const handleAdminReleaseCert = async (studentId: number, batchId: number, reason: string) => {
        if (!reason || !reason.trim()) {
            showToast("Please enter a valid reason for manual release", "error");
            return;
        }
        setFormLoading(true);
        try {
            const token = localStorage.getItem("snagup_token");
            const res = await axios.post(`${API_ENDPOINTS.CERTIFICATES}/admin/release`, {
                student_id: studentId,
                batch_id: batchId,
                release_reason: reason.trim()
            }, { headers: { Authorization: `Bearer ${token}` } });

            showToast(res.data.message || "Certificate manually released!", "success");
            setReleaseCertModal(null);
            loadData(activeTab);
        } catch (err: any) {
            showToast(err.response?.data?.error || "Failed to release certificate", "error");
        } finally {
            setFormLoading(false);
        }
    };
